...
public class Customer extends AbstractPersistable {
    ...
    @JsonIgnore
    public long getDistanceFromNextStandstill() {
        if (vehicle == null) {
            throw new IllegalStateException(
                    "This method must not be called when the shadow variables are not initialized yet.");
        }
        ...
        return nextCustomer.getLocation().getDistanceTo(location);
    }

    @JsonIgnore
    public int getDistanceFromNextStandstillHard() {
        if (vehicle == null) {
            throw new IllegalStateException(
                    "This method must not be called when the shadow variables are not initialized yet.");
        }

        if(nextCustomer.getLocation().getLatitude() == location.getLatitude() && nextCustomer.getLocation().getLongitude() == location.getLongitude()) {
            return 100;
        }
    ...
    @JsonIgnore
    public int vehicleHasDepot() {
        if (this.getNextCustomer() == null && this.getVehicle().getDepot()==null) {
            return 100;
        }
        else {
            return 0;
